...
impl MainMenu {
    pub fn draw(canvas: &mut WindowCanvas) -> Result<(), String> {
        ...
        draw_vertical_center_text(canvas, info, 48, Color::GRAY, 200)?;
        let info = String::from("Start Game (ENTER)");
        draw_vertical_center_text(canvas, info, 36, Color::GRAY, 260)?;
        let info = String::from("Developed with Rust and SDL2");
        draw_vertical_center_text(canvas, info, 16, Color::GRAY, 350)?;
        let info = String::from("BSŞ - 2024");
        draw_vertical_center_text(canvas, info, 16, Color::GRAY, 370)?;
