...
pub fn sys_player_movement(
    ...
    for (mut transform, player) in &mut player {
        let velocity = player.speed * time.delta_secs();
        if input.pressed(KeyCode::KeyW)
            || input.pressed(KeyCode::ArrowUp) && transform.translation.y <= WINDOW_HEIGHT / 6.5
        {
            transform.translation.y += velocity;
        }
        if input.pressed(KeyCode::KeyS)
            || input.pressed(KeyCode::ArrowDown) && transform.translation.y >= -WINDOW_HEIGHT / 6.
        {
            transform.translation.y -= velocity;
        }
        if input.pressed(KeyCode::KeyD)
            || input.pressed(KeyCode::ArrowRight) && transform.translation.x <= WINDOW_WIDTH / 22.
        {
            transform.translation.x += velocity;
        }
        if input.pressed(KeyCode::KeyA)
            || input.pressed(KeyCode::ArrowLeft) && transform.translation.x >= -WINDOW_WIDTH / 6.
        {
            transform.translation.x -= velocity;
        }
    }
...
pub fn sys_customer_movement(mut customers: Query<(&mut Transform, &Customer)>, time: Res<Time>) {
    for (mut transform, customer) in &mut customers {
        if transform.translation.x >= WINDOW_WIDTH / 9. && !customer.is_get {
            let velocity = customer.speed * time.delta_secs();
            transform.translation.x -= velocity;
        }
    }
}
...
pub fn sys_show_scoreboard(
    ...
    for mut text in &mut query {
        **text = format!("Kasa: {}", game_state.balance);
    }
}
...
fn get_random_donut() -> DonutType {
    ...
    let number = rng.random_range(0..=2);
    donuts[number]
}
