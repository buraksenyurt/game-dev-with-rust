    ...
    if let Ok(window) = window.single() {
        ...
        commands
            ...
            .with_children(|parent| {
                parent.spawn((
                    Mesh2d(meshes.add(Mesh::from(Rectangle::new(20.0, 1.0)))),
                    MeshMaterial2d(materials.add(ColorMaterial::from(TRIANGLE_COLOR))),
                    Transform {
...
pub fn move_forward_player(
    ...
    for (mut transform, mut velocity, mut direction) in &mut player_query {
        ...
        let translation = velocity.0 * timer.delta_secs();
