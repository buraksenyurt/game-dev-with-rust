...
pub fn get_random_fuel_station(
    ...
    let fuel_idx = rand::rng().random_range(0..fuel_amounts.len());
    let fuel_amount = fuel_amounts[fuel_idx];

    let speeds = [100., 125., 150.];
    let speed_idx = rand::rng().random_range(0..speeds.len());
...
pub fn spawn_fuel_station(
    ...
    commands.spawn((
        Sprite {
            image: asset_server.load(station.asset_file),
            ..default()
        },
        Transform::from_xyz(station.x, station.y, 0.),
        FuelStation {
