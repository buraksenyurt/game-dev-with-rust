...
pub fn get_random_meteor(window_query: Query<&Window, With<PrimaryWindow>>) -> MeteorValue {
    ...
    let idx = rand::rng().random_range(0..meteors.len());
    ...
    let idx = rand::rng().random_range(0..speeds.len());
    let speed = speeds[idx];

    let required_hit_counts = [1, 2, 3];
    let idx = rand::rng().random_range(0..required_hit_counts.len());
...
fn spawn_one_meteor(
    ...
    commands.spawn((
        Sprite {
            image: asset_server.load(meteor.asset_file),
            ..default()
        },
        Transform::from_xyz(meteor.x, meteor.y, 0.),
        Meteor {
...
pub fn move_meteors(mut query: Query<(&mut Transform, &Meteor)>, time: Res<Time>) {
    for (mut transform, meteor) in query.iter_mut() {
        ...
        transform.translation += direction * meteor.speed * time.delta_secs();
        transform.rotate_z(f32::to_radians(METEOR_ROTATE_DEGREE) * time.delta_secs());
    }
}
