...
fn main() {
    let mut quiz = WareHouse::load_quiz(Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/questions.json"))).expect("System problem.");
...
mod test {
    ...
    fn should_first_question_loaded() {
        let quiz = WareHouse::load_quiz(Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/questions.json"))).unwrap();
        assert!(quiz.len() > 0);
        assert_eq!(
            quiz[0].title,
